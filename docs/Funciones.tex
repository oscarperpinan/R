...
\begin{frame}[fragile,label={sec:orgheadline22}]{Analizar antes de que ocurra: \texttt{debug}}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:orgheadline24}]{Analizar antes de que ocurra: \texttt{trace}}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:orgheadline34}]{\texttt{Reduce}}
 \lstset{language=R,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
## (((1/2)/3)/4)/5
Reduce('/', 1:5)
...
\begin{frame}[fragile,label={sec:orgheadline35}]{Funciones recursivas}
 \begin{itemize}
